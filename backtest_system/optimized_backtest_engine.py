"""
OptimizedBacktestEngine 优化回测引擎模块
============================================
基于 Backtrader 核心，提供高性能的回测接口。
...
class OptimizedBacktestEngine:
    """
    优化回测引擎 —— 封装 Backtrader 核心流程，提供高性能回测
    ...
    def __init__(self, config: Optional[BacktestConfig] = None):
        ...
        logger.info("OptimizedBacktestEngine 初始化完成")
    ...
    def run_backtest(
        ...
        # 预处理数据
        if data_dict:
            self._preprocess_data(data_dict)
        ...
        cerebro.addstrategy(OptimizedStrategyAdapter, **default_params)
    ...
    def optimize(
        ...
        # 使用多进程进行参数优化
        cerebro = bt.Cerebro(maxcpus=self.config.cerebro_kwargs.get('maxcpus', None))
        ...
        results = cerebro.run(maxcpus=self.config.cerebro_kwargs.get('maxcpus', None))
    ...
    def _preprocess_data(self, data_dict: Dict[str, pd.DataFrame]):
        """
        预处理数据，提高回测性能
        ...
        Args:
            data_dict: {股票代码: DataFrame} 行情数据
        """
        logger.info(f"开始预处理 {len(data_dict)} 只股票数据...")
            ...
            with ThreadPoolExecutor(max_workers=min(8, len(data_dict))) as executor:
                ...
                for future in futures:
                    ...
                    try:
                        ...
                        self._processed_data[code] = processed_df
                    except Exception as e:
                        logger.error(f"预处理 {code} 数据失败: {e}")
        else:
            # 小数据集使用串行处理，避免并行开销
            for code, df in data_dict.items():
                try:
                    processed_df = self._process_single_stock(code, df)
                    self._processed_data[code] = processed_df
                except Exception as e:
                    logger.error(f"预处理 {code} 数据失败: {e}")
...
class OptimizedStrategyAdapter(BacktraderStrategyAdapter):
    """
